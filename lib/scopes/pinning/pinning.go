...
// StrongValidate performs strong validation of a scope pin.
...
// WeakValidate performs basic validation of a scope pin.
func WeakValidate(pin *scopesv1.Pin) error {
	...
	// perform any checks if any scopes are malformed as we cannot determine wether or not the assignments
	// within that scope aught to apply to the scope of the resource being targeted, and we cannot fallback
...
// AssignmentsForResourceScope returns a sequence of pinned assignments relevant to the target resoruce scope, starting
