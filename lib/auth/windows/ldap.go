...
// CRLDistributionPoint computes the CRL distribution point for certs issued TODO TODO.
func CRLDistributionPoint(activeDirectoryDomain string, caType types.CertAuthType, issuer *tlsca.CertAuthority) string {
	id := issuer.Cert.Subject.CommonName + "-" + hex.EncodeToString(issuer.Cert.SubjectKeyId)
