...
func (a *AuthCommand) GenerateCRLForCA(ctx context.Context, clusterAPI authCommandClient) error {
		...
		for _, authority := range authorities {
			for i, keypair := range authority.GetActiveKeys().TLS {
				if len(keypair.CRL) == 0 {
					// TODO: for backwards compatibility, generate a CRL: clusterAPI.GenerateCertAuthorityCRL(ctx, certType)
					return trace.NotImplemented("keypair %v is missing CRL for %v authority %v", i, authority.GetType(), authority.GetName())
				}
	...
	// TODO: handle missing CRL case here too
	fmt.Println(string(authorities[0].GetActiveKeys().TLS[0].CRL))
	return nil
}
