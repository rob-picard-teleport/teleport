...
1. Add the configuration for the Windows Desktop Service to the `/etc/teleport.yaml`
   ...
   windows_desktop_service:
     ...
     ldap:
       ...
       # If you have more than one LDAP server, you can
       # enable `locate_server` instead of addr to get
       # LDAP servers from DNS SRV records of your domain.
       locate_server: 
         enabled: true
         # Optional: specify a logical site to get LDAP servers from.
         site: "$LDAP_SITE_NAME"
   ...
   More information on the differences between the `addr` and `locate_server` options
   is available in the [LDAP server selection](#ldap-server-selection) section.
