...
func EventToGRPC(in types.Event) (*proto.Event, error) {
	...
	case types.Resource153UnwrapperT[*processhealthv1.ProcessHealth]:
		out.Resource = &proto.Event_ProcessHealth{
			ProcessHealth: r.UnwrapT(),
		}
	default:
...
func EventFromGRPC(in *proto.Event) (*types.Event, error) {
	...
	if r := in.GetResourceHeader(); r != nil {
		...
		out.Resource = types.Resource153ToLegacy(r)
		return &out, nil
	} else if r := in.GetProcessHealth(); r != nil {
		out.Resource = types.Resource153ToLegacy(r)
		return &out, nil
	} else {
